...
import com.smartpos.webviewapidemo.common.Common;
...
import com.smartpos.webviewapidemo.util.StringUtility;
...
//        String userKey = "CF 5F B3 6E 81 DD A3 C5 B2 D9 F7 3B D9 FF C0 48";
    ...
    public void listenForPinBlockWithMode(final Map<String, Object> param, final ActionCallback callback) {
        byte[] arryIVBuffer = StringUtility.StringToCreateByteArray("16 90 83 E9 6F 7C 00 00");
        ...
        try {
            OperationListener listener = new OperationListener() {
                ...
                public void handleResult(OperationResult arg0) {
                    if (arg0.getResultCode() == OperationResult.SUCCESS) {
                        byte[] pinBlock = ((PINPadOperationResult) arg0).getEncryptedPINBlock();
                        sendSuccessLog2("KEY_TYPE_MK_SK PINBlockWithMode = " + StringUtility.byteArray2String(pinBlock));
                    } else {
                        sendFailedOnlyLog(mContext.getString(R.string.operation_failed));
                    }
                }
            };
            device.listenForPinBlock(keyInfo, pan, false, listener, TimeConstants.FOREVER, MODE_ISP, arryIVBuffer);
            sendSuccessLog("");
        } catch (DeviceException e) {
            e.printStackTrace();
            sendFailedLog(mContext.getString(R.string.operation_failed));
    ...
    public void waitForPinBlockWithMode(final Map<String, Object> param, final ActionCallback callback) {
        byte[] arryIVBuffer = StringUtility.StringToCreateByteArray("16 90 83 E9 6F 7C 00 00");
        KeyInfo keyInfo = new KeyInfo(PINPadDevice.KEY_TYPE_MK_SK, 0, 0, AlgorithmConstants.ALG_DES);
        String pan = "4012345678909";
        try {
            sendSuccessLog("");
            OperationResult operationResult = device.waitForPinBlock(keyInfo, pan, false,
                    TimeConstants.FOREVER, MODE_ISP, arryIVBuffer);
            if (operationResult.getResultCode() == OperationResult.SUCCESS) {
                byte[] pinBlock = ((PINPadOperationResult) operationResult).getEncryptedPINBlock();
                sendSuccessLog2("KEY_TYPE_MK_SK PINBlockWithMode = " + StringUtility.byteArray2String(pinBlock));
            } else {
                sendFailedOnlyLog(mContext.getString(R.string.operation_failed)+operationResult.getResultCode());
            }
        } catch (DeviceException e) {
            e.printStackTrace();
            sendFailedLog(mContext.getString(R.string.operation_failed));
